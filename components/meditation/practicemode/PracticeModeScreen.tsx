...
export default function MeditationPracticeModeScreen() {
    ...
    const audioVariants = useMemo(() => {
        const singles = (meditation?.audioSingleSessions || [])
            .filter((item: any) => item.contentTypeOfAudio === 'meditation')
            .map((item: any) => ({ ...item, kind: 'single' as const }));

        const fromPlaylists = (meditation?.audio_playlists || []).map((playlist: any) => ({
    ...
    const videoVariants = useMemo(() => {
        const singles = (meditation?.videoSingleSessions || [])
            .filter((item: any) => item.contentTypeOfVideo === 'meditation')
            .map((item: any) => ({ ...item, kind: 'single' as const }));

        const fromPlaylists = (meditation?.video_playlists || []).map((playlist: any) => ({
            documentId: playlist.documentId,
            title: playlist.title,
            description: playlist.description,
        ...
        <View style={[styles.screen, { backgroundColor: colors.background }]}>
            ...
            >
                <View style={styles.headerBlock}>
                    <Text style={styles.mudraName}>{meditation?.name}</Text>
                    <Text style={styles.mudraSubtitle} numberOfLines={2}>
                        {meditation?.introCard?.introCardText}
                    </Text>
                </View>
                ...
                {activeVariants.length === 0 ? (
                    <View style={styles.emptyState}>
                        ...
                        <Text style={styles.emptyStateText}>
                            No {activeTab} sessions available for this meditation yet.
                        </Text>
                    </View>
                ) : (
