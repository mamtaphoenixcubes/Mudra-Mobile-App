...
export default function PranayamaPracticeModeScreen() {
    ...
    // TODO: replace with usePranayamaStore's selectedPranayama once the store exists
    const pranayama: any = undefined;

    const audioVariants = useMemo(() => {
        const singles = (pranayama?.audioSingleSessions || [])
            .filter((item: any) => item.contentTypeOfAudio === 'pranayama')
            .map((item: any) => ({ ...item, kind: 'single' as const }));

        const fromPlaylists = (pranayama?.audio_playlists || []).map((playlist: any) => ({
    ...
    const videoVariants = useMemo(() => {
        const singles = (pranayama?.videoSingleSessions || [])
            .filter((item: any) => item.contentTypeOfVideo === 'pranayama')
            .map((item: any) => ({ ...item, kind: 'single' as const }));

        const fromPlaylists = (pranayama?.video_playlists || []).map((playlist: any) => ({
            documentId: playlist.documentId,
            title: playlist.title,
            description: playlist.description,
        ...
        <View style={[styles.screen, { backgroundColor: colors.background }]}>
            ...
            >
                <View style={styles.headerBlock}>
                    <Text style={styles.mudraName}>{pranayama?.name}</Text>
                    <Text style={styles.mudraSubtitle} numberOfLines={2}>
                        {pranayama?.introCard?.introCardText}
                    </Text>
                </View>
                ...
                {activeVariants.length === 0 ? (
                    <View style={styles.emptyState}>
                        ...
                        <Text style={styles.emptyStateText}>
                            No {activeTab} sessions available for this pranayama yet.
                        </Text>
                    </View>
                ) : (
