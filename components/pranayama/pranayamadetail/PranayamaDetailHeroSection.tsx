...
const fallbackPranayama = {
    name: 'Nadi Shodhana',
    intentions: ['Balance', 'Calm', 'Focus'],
...
type Props = { pranayama?: any };

export default function PranayamaDetailHeroSection({ pranayama }: Props) {
    const { colors, isDark } = useTheme();
    const data = pranayama ?? fallbackPranayama;
    ...
    const details = [
        { label: 'Duration', value: data.duration || '10 min', Icon: isDark ? ClockWhite : ClockSvg },
        { label: 'Level', value: data.level || 'Beginner', Icon: isDark ? GroupWhite : GroupSvg },
        { label: 'Technique', value: data.technique || 'Breathing', Icon: isDark ? LanguageWhite : LanguageSvg },
    ];
